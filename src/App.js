...
class App extends Component {
  ...
  renderChart() {
    ...
    var svg = d3.select(".svg_parent").attr("height", 250).attr("width", 1000)
    
    var xPos = d3.scaleLinear().domain([0, data.length]).range([10, 990]);
    var fontSize = d3.scaleLinear().domain([d3.min(data, d => d[1]), d3.max(data, d => d[1])]).range([16, 56]);
    // svg.selectAll(".word").data(data).join("text").attr("class", "word").attr("y", 130).transition().duration(2000).text(d => d[0]).attr("x", (d, ind) => xPos(ind));

    svg.selectAll(".word").data(data, d => d[0]).join(
        enter => enter.append("text").attr("class", "word").attr("y", 130).text(d => d[0]).attr("x", (d, ind) => xPos(ind)).style("font-size", "0px").transition().duration(3000).style("font-size", d => `${fontSize(d[1])}px`),
        update => update.transition().duration(3000).text(d => d[0]).attr("x", (d, ind) => xPos(ind)).style("font-size", d => `${fontSize(d[1])}px`),
        exit => exit.remove()
    )
  }
